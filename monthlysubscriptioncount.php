  ...
  header("Location:login.php");
		...
		if ($_POST['submit']){
		  ...
		  while( $row = mysqli_fetch_array($result)) {
			  ...
			  $subtime = $row['subtime'];
			  $endtime = $row['endtime'];
			  //$query = "SELECT * FROM subscription WHERE MONTH(datereg)='$selmonth' AND YEAR(datereg)='$selyear' AND subcode='$subcode'";
			  //$query = "SELECT * FROM subscription WHERE MONTH(from_unixtime(subtime,'%Y%m%d'))='$selmonth' AND YEAR(from_unixtime(subtime,'%Y%m%d'))='$selyear' AND subcode='$subcode'";
			  $query = "SELECT * FROM subscription WHERE subtime>='$startdate' AND endtime<='$enddate' AND subcode='$subcode'";
			  $totalrows = $db_handle->numRows($query);
			$message_success = "Total active subscriptions for the month of $specperiod ON Service($services_name) is $totalrows times<br>";
			$totsubcr += $totalrows;

		  }
			$message_success .= "Total subscriptions for the month of $specperiod is $totsubcr";
